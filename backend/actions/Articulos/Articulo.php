...
class Articulo extends gvHidraForm_DB
{
	public function __construct() {
		...
		$str_select = <<<query
		SELECT idarticulo as "lis_idarticulo",
			articulo.nombre as "lis_nombre",
			articulo.descripcion as "lis_descripcion",
			articulo.precio as "lis_precio",
			articulo.imagen as "lis_imagen",
    		articulo.stock as "lis_stock",
      		categoria.nombre as "lis_categoria" 
      FROM articulo INNER JOIN categoria
      ON articulo.categoria = categoria.idcategoria
query;
	...
	*/	
	public function preBorrar($objDatos) {
		$idArticulo = $objDatos->getValue("lis_idarticulo");

		$query = <<<query
SELECT count(*) as "resultados" FROM linea_pedido WHERE idarticulo = $idArticulo 
query;

	$resultados = $this->consultar($query);
	$numero_resultados = intval($resultados[0]['resultados']);
		if($numero_resultados > 0){
			$this->showMessage('APL-5');
			return -1;
		}else{
			return 0;
		}
