...
class Categoria extends gvHidraForm_DB
{
    ...
    {
        $categoria = $objDatos->getValue("edi_nombre");
    ...
    {
        $idCategoria = intval($objDatos->getValue("lis_idcategoria"));

        $query = <<<query
SELECT count(*) as "conteo" FROM articulo WHERE categoria = $idCategoria
query;
        $resultado = $this->consultar($query);
        $conteo = intval($resultado[0]['conteo']);
        if($conteo > 0){
            $this->showMessage('APL-6');
            return -1;
        }
